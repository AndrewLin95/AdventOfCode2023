...
fs.readFile('Day1_Input.txt', 'utf-8', (err, data) => {
  if (err) throw err;
  ...
  formattedData.forEach((string) => {
    ...
    stringArray.forEach((char) => {
      ...
      if (firstNum === null) {
        if (/\d/.test(char)) {
          ...
          /(one|two|three|four|five|six|seven|eight|nine)/.test(currString)
        ) {
    ...
    const formattedNumber = parseInt(`${firstNum}${lastNum}`, 10);
